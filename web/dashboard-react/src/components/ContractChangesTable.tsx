...
export function ContractChangesTable({ title, rows, useDeltaRows = false }: Props) {
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900 p-3">
      <div className="mb-2 flex flex-wrap items-center gap-2">
        ...
        <input className="rounded bg-slate-800 px-2 py-1 text-xs" placeholder="筛选 ticker / 合约" value={keyword} onChange={(e) => setKeyword(e.target.value)} />
            ...
            {filtered.slice(0, 250).map((r, idx) => {
              ...
              return (
                <React.Fragment key={id}>
                  <tr className="cursor-pointer border-b border-slate-800 hover:bg-slate-800/60" onClick={() => setExpanded((s) => ({ ...s, [id]: !s[id] }))}>
                    ...
                    <td className="px-2 py-2"><span className="rounded bg-cyan-500/20 px-2 py-0.5 text-[11px] text-cyan-300">{status}</span></td>
                  </tr>
                  {expanded[id] && (
