...
export default function App() {
  ...
  return (
    <div className="mx-auto max-w-[1700px] space-y-6 p-4">
      <header className="rounded-2xl border border-slate-800 bg-slate-900 p-4">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold text-emerald-300">异常期权盘中监控面板（React 版）</h1>
            <p className="text-xs text-slate-400">数据库只读：{dbPath}</p>
          </div>
          <div className="flex items-center gap-2 text-sm">
            <button className={`rounded px-3 py-1 ${table === 'stock' ? 'bg-cyan-500 text-black' : 'bg-slate-700'}`} onClick={() => setTable('stock')}>Stock</button>
            <button className={`rounded px-3 py-1 ${table === 'etf' ? 'bg-cyan-500 text-black' : 'bg-slate-700'}`} onClick={() => setTable('etf')}>ETF</button>
          </div>
        </div>
