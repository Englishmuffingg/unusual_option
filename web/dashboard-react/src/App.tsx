...
export default function App() {
  ...
  const newRows = useMemo(
    () => filteredRows.filter((r) => r.isNew).sort((a, b) => b.optionsVolume - a.optionsVolume).slice(0, ACTIVITY_TOP_N),
    [filteredRows],
  )
      ...
      {!loading && !error && rows.length > 0 && (
        <>
          ...
          <section className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <ActivityPanel
              title="新增活动"
              description={`当前仅展示在已筛选结果中的前 ${ACTIVITY_TOP_N} 条（按成交量降序）`}
              rows={newRows}
...
function ActivityPanel({
  ...
  return (
    <section className={CARD_CLASS}>
      ...
      {description ? <p className="mb-3 mt-1 text-xs text-slate-500">{description}</p> : null}
      {rows.length === 0 ? (
